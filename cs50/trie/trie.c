...
int main(int argc, char *argv[])
{
    ...
    root->is_word = false;//устанавливается значение свойства is_word на false для переменной root.
...
bool check(char* word)
{
    ...
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        ...
        if (cursor->children[index] == NULL)
        {
            // If the current letter does not exist in the trie, the word is not in the trie
            return false;
        }

        // If the letter exists in the trie, move to the next node
        cursor = cursor->children[index];
    }

    // If we've gone through all the letters in the word and reached a node that is marked as a word, the word exists in the trie
    //return cursor->is_word && cursor != root;
